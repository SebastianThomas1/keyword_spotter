...
def to_mfcc(file: Union[str, PathLike], n_mfcc: int = 13,
...
def to_features(file: Union[str, PathLike], n_mfcc: int = 13,
                hop_length: int = 512, n_fft: int = 2048) -> tf.Tensor:
    ...
    return tf.expand_dims(to_mfcc(file, n_mfcc=n_mfcc, hop_length=hop_length,
                                  n_fft=n_fft), -1)


def to_label(path: PurePath) -> tf.Tensor:
